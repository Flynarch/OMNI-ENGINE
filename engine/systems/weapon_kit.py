...
def _classify_kind(tags: list[str], item_id: str, wprof: dict[str, Any]) -> str | None:
    if isinstance(wprof, dict) and str(wprof.get("kind") or "").strip():
        ...
        if k in ("firearm", "melee"):
            return k
    # Ammunition SKUs (e.g. ammo_9mm) must not register as equippable firearms.
    if "ammo" in tags:
        return None
...
def ensure_weapon_for_item(state: dict[str, Any], item_id: str, *, display_name: str | None = None, source: str = "") -> bool:
    ...
    if kind == "firearm":
        ...
        aid = str(wprof.get("ammo_item_id", "ammo_9mm") or "ammo_9mm").strip()
        base = {
            "name": str(name),
            "kind": "firearm",
            "ammo": fill,
        ...
        if existing and src == "buy":
            ...
            for k in ("ammo", "mag_capacity", "condition_tier", "use_count", "jammed", "ammo_item_id"):
                if k in existing:
                    base[k] = existing[k]
