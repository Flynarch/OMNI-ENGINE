    ...
    *,
    ...
    for ev in state.get("pending_events", []):
        if not isinstance(ev, dict) or ev.get("triggered"):
            continue
        if (_safe_int(ev.get("due_day", 99999), 99999), _safe_int(ev.get("due_time", 99999), 99999)) <= (cur_day, cur_min):
    ...
    for rp in state.get("active_ripples", []):
        if not isinstance(rp, dict) or rp.get("surfaced"):
            continue
        if (_safe_int(rp.get("surface_day", 99999), 99999), _safe_int(rp.get("surface_time", 99999), 99999)) <= (cur_day, cur_min):
            due_ripples.append(rp)

    items: list[tuple[tuple[int, int], str, dict[str, Any]]] = []
    for ev in due_events:
        items.append(((_safe_int(ev.get("due_day", 99999), 99999), _safe_int(ev.get("due_time", 99999), 99999)), "event", ev))
    for rp in due_ripples:
        items.append(((_safe_int(rp.get("surface_day", 99999), 99999), _safe_int(rp.get("surface_time", 99999), 99999)), "ripple", rp))
